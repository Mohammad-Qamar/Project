...
function ViewNoticeBoard() {
  ...
  useEffect(() => {
    const fetchnoticeboards = async () => {
      try {
        setData((data) => ({
          noticeboards: data.noticeboards,
          isFetching: true,
        }));
        const response = await axios.get(
          `http://localhost:8080/faculty/viewnoticeboard/${sessionStorage.getItem(
            "userId"
          )}`
        );
  ...
  const removeNoticeBoard = (id) => {
    axios
      .delete(`http://localhost:8080/faculty/viewnoticeboard/delete/${id}`)
      .then((response) => {
                    ...
                    <tr>
                      ...
                      <td>
                        <button
                          ...
                          onClick={() =>
                            navigate(`/faculty/editnoticeboard/${id}`)
                          }
                        >
                          Edit
                        </button>
